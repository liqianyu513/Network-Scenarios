...
nodes = {
    "Website": m.NodeInfo(
        ...
        # database user"  如果可以SSH到服务器，然后获得FLAG共享凭据与数据库用户
        properties=["MySql", "Ubuntu", "nginx/1.10.3", "HTTPS", "SSH"],
        owned_string="FLAG: Login using insecure SSH user/password",
        vulnerabilities=dict(
            ScanPageContent=m.VulnerabilityInfo(
                description="LeakedGitHubProjectUrl: Website page content shows a link to GitHub "  #网站页面内容显示一个到GitHub的链接
                            "repo",
                type=m.VulnerabilityType.REMOTE,
                outcome=m.LeakedNodesId(["GitHubProject"]),
                reward_string="WEBSITE page content has a link to github -> Github project discovered!", #网站页面内容有一个链接到github ->发现github项目!
                cost=1.0
            ),
            ScanPageSource=m.VulnerabilityInfo(
                description="Website page source contains refrence to browseable "  #网站页面源包含可浏览的相对网络目录的参考
            ...
            CredScanBashHistory=m.VulnerabilityInfo(
                description="bash history leaking creds - FLAG Stealing "
                            "credentials for the monitoring user", #监视用户窃取凭证
    ...
    "Website.Directory": m.NodeInfo(
        ...
        properties=["Ubuntu", "nginx/1.10.3",
                    "CTFFLAG:Readme.txt-Discover secret data", "HTTPS"
                    ],
        vulnerabilities=dict(
    ...
    "Website[user=monitor]": m.NodeInfo(
        ...
        value=100,
        properties=["MySql", "Ubuntu", "nginx/1.10.3", "SSH", "SSH-key", "su"],
    ...
    "GitHubProject": m.NodeInfo(
        ...
        value=10,
        properties=["GitHub", "SasUrlInCommit", "GIT"],
        vulnerabilities=dict(
    ...
    "AzureStorage": m.NodeInfo(
        ...
        value=50,
        properties=["CTFFLAG:LeakedCustomerData", "HTTPS"],
        vulnerabilities=dict(
    ...
    'Sharepoint': m.NodeInfo(
        services=[m.ListeningService("HTTPS")],
        value=100,
        properties=["SharepointLeakingPassword", "HTTPS"],
    ...
    'AzureResourceManager': m.NodeInfo(
        ...
        value=50,
        properties=["CTFFLAG:LeakedCustomerData2", "HTTPS"],
        vulnerabilities=dict(
    ...
    'AzureVM': m.NodeInfo(
        ...
        value=100,
        properties=["CTFFLAG:VMPRIVATEINFO", "PING", "SSH"],
        firewall=m.FirewallConfiguration(
            incoming=[m.FirewallRule("SSH", m.RulePermission.BLOCK,
...
# global_node_list: Dict[NodeID, NodeInfo] = dict([])
...
ENV_IDENTIFIERS = m.default_identifiers()
# global_nodes = dict([])
# print(global_nodes)
...
# def new_environment() -> m.Environment:
#     return m.Environment(
#         network=w.EnvironmentBounds.of_identifiers(),
#         vulnerability_library=global_vulnerability_library,
#         identifiers=ENV_IDENTIFIERS
#     )
